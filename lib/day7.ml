...
let rec compare_hands h1 h2 =
  ...
  | s1 :: r1, s2 :: r2 ->
    (match compare s1 s2 with
     | x when x == 0 -> compare_hands r1 r2
     | x -> x)
  | _ -> 0
;;
...
let rec parse_players () =
  ...
  | [ cards; amount ] -> (parse_cards cards, int_of_string amount) :: parse_players ()
  | _ -> []
;;
...
let part1 () =
  ...
  (* let _, y = List.split players in
     Utils.print_list y; *)
  let a, _ = List.fold_left (fun (c, r) (_, s) -> c + (s * r), r + 1) (0, 1) players in
  a
;;
