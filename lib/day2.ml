...
let parse_set set_str =
    let rec parse_set' set_list carry =
        ...
        | x :: rest -> let new_ret = 
            ...
            | _ :: amount :: color :: _ ->
                let new_ret = match color with
                | "red" -> {r = int_of_string amount; b = carry.b; g = carry.g}
                | "green" -> {g = int_of_string amount; b = carry.b; r = carry.r}
                | "blue" -> {b = int_of_string amount; g = carry.g; r = carry.r}
                | _ -> carry
                in
                new_ret
            | _ -> carry
...
let check_line line =
    ...
    | game :: rest :: [] -> let id = match String.split_on_char ' ' game with
        ...
        let resp = match sets_ok sets with
        | true -> Some id
        | false -> None
        in
        resp
