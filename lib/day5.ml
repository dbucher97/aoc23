...
let rec parse_map items =
  ...
  | [ a; b; c ] ->
    parse_map
      ({ f = int_of_string a; t = int_of_string b; range = int_of_string c } :: items)
  | _ -> items
;;
...
let rec follow_all maps i =
  ...
  match maps with
  | [] -> i
  | map :: rest -> follow_all rest (follow i map)
;;
...
let parse_seed_ranges () =
  ...
  let rec parse_seed_ranges' l =
    match l with
    | a :: b :: rest -> (a, b) :: parse_seed_ranges' rest
    | _ -> []
  in
...
let rec range_follow map (start, n) =
  ...
  | { f; t; range } :: rest ->
    (match start - t with
     ...
     | x when is_left start n t range -> (f, n + x) :: range_follow rest (start, -x)
...
let range_follow_all maps sources = List.fold_left range_follow_multi sources maps
